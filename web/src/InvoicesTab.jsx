...
export default function InvoicesTab({ tenantId, companyName }) {
  ...
  async function loadData() {
    ...
    setError("");
    try {
      const [invSnap, custSnap, accSnap, itemSnap, tenantSnap] = await Promise.all([
      ...
      // الرقم الضريبي للبائع (من إعداد الشركة، إن وُجد)
      if (!tenantSnap.empty) {
  ...
  return (
    <div>
      ...
      {customers.length === 0 ? (
        <div style={styles.notice}>أضف عميلًا أولًا (من تبويب «العملاء») قبل إنشاء فاتورة.</div>
